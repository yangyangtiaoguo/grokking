...
\item A family-discrimination analysis of the boundary's shape, validated by a pre-specified held-out prediction test (\S\ref{sec:results-family}).
\item Locally robust, pre-specified directional predictions across width, learning rate, and optimizer choice (\S\ref{sec:results-robustness}), and transfer of the injection-ranking direction to a second arithmetic task, modular addition, under sample-count-matched controls (\S\ref{sec:results-modadd}).
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{Deconfounded dose sweep: batch size (128) and step count ($60{,}000$) fixed throughout; expected dose $q/128$ implemented by probabilistic single-slot replacement. Pooled across $\alpha\in\{2.5,2.75,3.0\}$, 24 seeds per dose point.}
\label{tab:doseclean}
\begin{tabular}{lccc}
\toprule
...
\begin{tabular}{lcccccc}
\toprule
Work & Skew axis & Finite-budget map & Rescue controls & Exposure match & Task transfer & Toy scale \\
\midrule
...
All results in this paper are established at $n\le 12$, $d\le 128$, within a single decoder-only Transformer architecture family. The family-discrimination analysis in \S\ref{sec:results-family} establishes a model class, not a closed-form law: the two best-performing candidates differ by a small margin at every held-out point.
...
All training code, the frozen verdict rule, per-seed result logs, and the pre-specified prediction files (Tables~\ref{tab:heldout} and \ref{tab:robustness}, recorded before the corresponding measurement runs per \S3.7) will be released in a public repository upon publication.
